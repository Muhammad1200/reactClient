...
import { selectContacts, selectLeadByName } from '../store/appFeed/selectors'
...
export default function AddLeadMUI() {
    ...
    if (addedLead && submitted) {
        history.push(`/leads/${addedLead.id}`)
    }
			...
			<Container component='main' maxWidth='xs'>
				<div className={classes.paper}>
					...
					<form className={classes.form} onSubmit={submitLeadForm} noValidate>
						...
						<TextField
							onChange={(event) =>
								set_associated_company_name(event.target.value)
							}
